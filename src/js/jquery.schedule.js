(function ($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        ...
        init: function () {
            ...
            if (this.settings.mode === "edit") {
                ...
                $(this.element).on("mousedown", ".jqs-day", function (event) {
                    ...
                    if (
                        !$(event.target).hasClass("jqs-period") &&
                        $(event.target).parents(".jqs-period").length === 0
                    ) {
                        var time = "";
                        ...
                        helper = $("<div>")
                            .addClass("jqs-period-helper")
                            .css({
                                height: $this.periodPosition,
                                top: position * $this.periodPosition,
                            })
                            .append(
                                '<div class="jqs-period-helper-time">' +
                                    time +
                                    "</div>"
                            );
                ...
                $(this.element).on("mouseenter", ".jqs-day", function () {
                    ...
                    $(".jqs-grid-day", $this.element)
                        .eq(index)
                        .addClass("jqs-grid-day-buttons");
                });
                ...
                $(this.element).on("mouseleave", ".jqs-day", function () {
                    ...
                    $(".jqs-grid-day", $this.element)
                        .eq(index)
                        .removeClass("jqs-grid-day-buttons");
                });
         ...
         */
        create: function () {
            $('<table class="jqs-table"><tr></tr></table>').appendTo(
                $(this.element)
            );

            for (var i = 0; i < this.settings.days; i++) {
                $('<td><div class="jqs-day"></div></td>').appendTo(
                    $(".jqs-table tr", this.element)
                );
            }
            ...
            if (this.settings.mode === "edit") {
                dayRemove =
                    ...
                    this.settings.periodRemoveButton +
                    '"></div>';
                dayDuplicate =
                    '<div class="jqs-day-duplicate" title="' +
                    this.settings.periodDuplicateButton +
                    '"></div>';
            }
        ...
        add: function (parent, position, height, options) {
            ...
            if (this.settings.mode === "edit") {
                periodRemove =
                    '<div class="jqs-period-remove" title="' +
                    this.settings.periodRemoveButton +
                    '"></div>';
                periodDuplicate =
                    '<div class="jqs-period-duplicate" title="' +
                    this.settings.periodDuplicateButton +
                    '"></div>';
            }

            var periodTitle =
                '<div class="jqs-period-title">' + options.title + "</div>";
            var periodTime =
                '<div class="jqs-period-time">' +
                this.periodInit(position, position + height) +
                "</div>";
            var period = $(
                '<div class="jqs-period">' +
                    '<div class="jqs-period-container">' +
                    periodTime +
                    periodTitle +
                    periodRemove +
                    periodDuplicate +
                ...
                period
                    .draggable({
                        ...
                        drag: function (event, ui) {
                            $(".jqs-period-time", ui.helper).text(
                                $this.periodDrag(ui)
                            );
                            $this.closeOptions();
                        },
                        stop: function (event, ui) {
                            if (!$this.isValid($(ui.helper))) {
                                ...
                                $(ui.helper).css(
                                    "top",
                                    Math.round(ui.originalPosition.top)
                                );
                            }
                        },
                    })
                ...
                if (this.settings.periodOptions) {
                    period.click(function (event) {
                        if (
                            ...
                            $this.settings.onClickPeriod.call(
                                this,
                                event,
                                period,
                                $this.element
        ...
        remove: function (period) {
            if (
                !this.settings.onRemovePeriod.call(this, period, this.element)
            ) {
                period.remove();
                this.closeOptions();
            }
        ...
        duplicate: function (period) {
            if (
                !this.settings.onDuplicatePeriod.call(
                    this,
                    period,
                    this.element
                )
        ...
        openOptions: function (event, period) {
            ...
            var titleInput =
                '<div class="jqs-options-title-container">' +
                '<input type="text" placeholder="' +
                this.settings.periodTitlePlaceholder +
                '" value="' +
                title +
                '" class="jqs-options-title"></div>';

            // color
            var colorInput = "";
            if (
                this.settings.periodColors &&
                this.settings.periodColors.length > 0
            ) {
                ...
                $.each(this.settings.periodColors, function (index, color) {
                    colorInput +=
                        '<button class="jqs-options-color" ' +
            ...
            // button
            var remove =
                '<div class="jqs-options-remove">' +
                this.settings.periodRemoveButton +
                "</div>";
            var duplicate =
                '<div class="jqs-options-duplicate">' +
                this.settings.periodDuplicateButton +
                "</div>";
        ...
        periodDrag: function (ui) {
            ...
            var end = Math.round(
                ($(ui.helper).height() + ui.position.top) / this.periodPosition
            );
        ...
        periodResize: function (ui) {
            var start = Math.round(ui.position.top / this.periodPosition);
            var end = Math.round(
                (ui.size.height + ui.position.top) / this.periodPosition
            );
        ...
        periodData: function (period) {
            ...
            var end = Math.round(
                (period.height() + period.position().top) / this.periodPosition
            );
            ...
            return {
                ...
                backgroundColor: $(".jqs-period-container", period).css(
                    "background-color"
                ),
                borderColor: $(".jqs-period-container", period).css(
                    "border-top-color"
                ),
        ...
        isValid: function (current) {
            ...
            var currentEnd = Math.round(
                current.position().top + current.height()
            );

            var start = 0;
            var end = 0;
            var check = true;
            $(".jqs-period", $(current).parent()).each(function (
                index,
                period
            ) {
                if (current.attr("id") !== $(period).attr("id")) {
                    start = Math.round($(period).position().top);
                        ...
                        $(period).position().top + $(period).height()
                    );
